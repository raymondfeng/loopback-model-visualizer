...
ModelNode.prototype.properties = function properties() {
  ...
  if(self._value !== loopback.DataModel) {
    superInstanceMethods = superInstanceMethods.concat(Object.keys(loopback.DataModel.prototype));
  }

  var instanceMethodDescriptors = [];
  if (!this._options.excludeInstanceMethods) {
  ...
  if(self._value !== loopback.DataModel) {
    superStaticMethods = superStaticMethods.concat(Object.keys(loopback.DataModel));
  }

  var staticMethodDescriptors = [];
  if (!this._options.excludeStaticMethods) {
